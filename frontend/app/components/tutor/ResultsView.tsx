...
export function ResultsView({ result, onReset }: Props) {
  ...
  return (
    ...
    >
      {/* Score header */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 mb-8 p-5 bg-brand-50 border border-brand-200 rounded-xl">
        <div>
        ...
        {results.map((r) => (
          ...
          >
            <div className="flex items-start gap-2.5 mb-1.5">
              <span className={`font-black text-[13px] shrink-0 mt-0.5 ${r.is_correct ? "text-green-600" : "text-red-500"}`}>
                {r.is_correct ? "✓" : "✗"}
              </span>
